...
def generate_cover_html(metadata: dict, theme: str, width: int, height: int) -> str:
    ...
    avatar = metadata.get('avatar', '')  # 头像图片路径；为空则用占位符
    date = metadata.get('date', '')
    avatar_text = metadata.get('avatar_text', '策')  # 占位符圆圈里的字
    ...
    is_flat = theme == 'default'  # default 单层平铺：内层透明、无圆角、无阴影
    ...
    # 头像 HTML：有 avatar 路径走 <img>，否则用 CSS 圆圈+文字占位符
    if avatar and os.path.exists(avatar):
        avatar_abs = os.path.abspath(avatar)
        avatar_html = f'<img class="cover-avatar" src="file://{avatar_abs}" alt="avatar">'
    else:
        avatar_html = f'<div class="cover-avatar cover-avatar-placeholder">{avatar_text}</div>'

    # 仅当 author/date 至少一项非空时渲染 header
    header_html = ''
    if author or date:
        header_html = f'''
    ...
    <style>
        * {{
        ...
        .cover-inner {{
            ...
            background: {'transparent' if is_flat else '#F3F3F3'};
...
def generate_card_html(content: str, theme: str, page_number: int = 1,
    ...
    bg = theme_backgrounds.get(theme, theme_backgrounds['default'])
    is_flat = theme == 'default'  # default 单层：内层透明无圆角无阴影
    inner_bg = 'transparent' if is_flat else 'rgba(255, 255, 255, 0.95)'
    inner_radius = '0' if is_flat else '20px'
    inner_shadow = 'none' if is_flat else '0 8px 32px rgba(0, 0, 0, 0.1)'
    inner_blur = 'none' if is_flat else 'blur(10px)'
...
async def render_markdown_to_cards(md_file: str, output_dir: str, 
    ...
    # 水印文本：优先取 xhs_id（格式化为「小红书号: xxx」），否则取 author，否则留空
    xhs_id = metadata.get('xhs_id', '')
    watermark = ''
    if xhs_id:
        watermark = f'小红书号: {xhs_id}'
    elif metadata.get('author'):
        watermark = f"@{metadata.get('author')}"
